...
export function AccountDetail({ accountId }: { accountId: string }) {
  ...
  const eligibility = useQuery({ queryFn: () => getAccountDeleteEligibility(accountId), queryKey: ["accounts", accountId, "delete-eligibility"] });
  ...
  return <MobileShell><div className="standard-page account-detail-page"><PageHeader backHref={"/accounts" as Route} title="Account details" />
    ...
    {value ? <><section className="account-detail-hero"><span><Landmark aria-hidden="true" size={25} /></span><p className="eyebrow">{value.type.replaceAll("_", " ")}</p><h2>{value.name}</h2><strong>{formatMoney(value.current_balance, value.currency)}</strong>{value.is_default ? <small><Star aria-hidden="true" size={13} /> Default account</small> : value.is_disabled ? <small className="account-disabled">Disabled</small> : <small>Available for transactions</small>}</section>
      ...
      <section className="account-actions" aria-label="Account actions">{!value.is_default && !value.is_disabled ? <button disabled={setDefault.isPending} onClick={() => void setDefault.mutateAsync(accountId)} type="button"><Star aria-hidden="true" size={18} />{setDefault.isPending ? "Updating…" : "Make default"}</button> : null}{!value.is_disabled ? <button disabled={disable.isPending} onClick={() => void disable.mutateAsync(accountId)} type="button"><ArchiveX aria-hidden="true" size={18} />{disable.isPending ? "Disabling…" : "Disable account"}</button> : null}{eligibility.data && canDeleteAccount(eligibility.data) ? <button className="account-delete-button" onClick={() => setDeleteOpen(true)} type="button"><Trash2 aria-hidden="true" size={18} />Remove account</button> : null}</section>
      <p className="account-detail-note"><ShieldCheck aria-hidden="true" size={16} />{eligibility.isPending ? "Checking whether this account can be removed…" : eligibility.isError ? "Removal is hidden until account activity can be verified." : eligibility.data && !canDeleteAccount(eligibility.data) ? accountDeleteReason(eligibility.data) : "This account has no linked activity and can be removed."}</p>
    </> : null}
