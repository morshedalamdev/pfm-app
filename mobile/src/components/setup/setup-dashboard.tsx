...
import { getDefaultAccount, isAccountActive } from "@/lib/accounts/utils";
...
export function SetupDashboard() {
  ...
  const defaultAccount = getDefaultAccount(activeAccounts);
  const selectedCurrency = defaultAccount?.currency || currency || profile.data?.base_currency || "USD";
  ...
  return <MobileShell><div className="standard-page setup-page"><PageHeader backHref={null} title="Set up your money" />
    ...
    {profile.data && accounts.data ? <div className="setup-steps"><section className="setup-step"><span className={`setup-step-number ${currencySaved || defaultAccount ? "setup-step-number--done" : ""}`}>{currencySaved || defaultAccount ? <Check aria-hidden="true" size={16} /> : "1"}</span><div><p className="eyebrow">YOUR HOME CURRENCY</p><h3>{defaultAccount ? `${defaultAccount.currency} is your system currency` : "What currency do you use most?"}</h3>{defaultAccount ? <p>Your system currency follows {defaultAccount.name}, your default account.</p> : <form className="setup-currency-form" onSubmit={(event) => void saveCurrency(event)}><input aria-label="Home currency" maxLength={3} onChange={(event) => setCurrency(event.target.value.toUpperCase())} value={selectedCurrency} /><button disabled={update.isPending} type="submit">{update.isPending ? "Saving…" : currencySaved ? "Saved" : "Save"}</button></form>}{error ? <p className="form-error" role="alert">{error}</p> : null}</div></section>
