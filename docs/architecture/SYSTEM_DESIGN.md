...
Phase 03.4 exposes transfers as an auditable API representation over paired source records. A transfer creates one positive `transfer_debit` row against the source account, one positive `transfer_credit` row against the destination account, and one `transfer_links` row connecting both sides. Same-currency transfers use the source amount on both rows. Cross-currency transfers require a positive decimal-string `converted_amount`; the source row stores the requested amount in the source currency and the destination row stores the converted amount in the destination currency. The response preserves `amount` and `currency` for the source side and returns `converted_amount` and `converted_currency` for the destination side. The service rejects same-account transfers, inactive or cross-user accounts, invalid amounts, timezone-naive dates, missing converted amounts for differing currencies, and converted amounts on same-currency transfers.
