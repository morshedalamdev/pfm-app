...
Phase 05.1 defines report contracts only. The endpoints below are documented for implementation in later milestone 05 phases; no report routes are mounted yet.

- Dashboard report: `GET /api/v1/reports/dashboard?period=week|month|year&type=income|expense&as_of=YYYY-MM-DD`. It returns available balance, selected-period income/expense/net-flow totals, and the RootChart bucket series for the selected `period` and transaction `type`.
- Monthly summary report: `GET /api/v1/reports/monthly-summary?month=YYYY-MM`. It returns the analytics page summary cards plus bounded top-expense and monthly-trend card data. Separate top-expense and monthly-trend endpoints are intentionally avoided until the UI needs independent pagination or filtering.
- Cash-flow report: `GET /api/v1/reports/cash-flow?date_from=<ISO datetime>&date_to=<ISO datetime>&interval=day|week|month`. It returns zero-filled income, expense, and net-flow buckets for line charts such as analytics income-vs-expense.
- Spending-by-category report: `GET /api/v1/reports/spending-by-category?date_from=<ISO datetime>&date_to=<ISO datetime>`. It returns chart-ready expense category slices with category id/name/icon, amount, and percent.
...
| Screen | Visible data | Required query | Required mutation | Validation | Loading state | Empty state | Error state |
|---|---|---|---|---|---|---|---|
| `/budget` | Month selector, monthly budget `$2,483.39`, spent `$2100.00`, remaining `$1400.00`, percent used, days remaining, repeated category cards | Implemented: `GET /api/v1/budgets?month=YYYY-MM`; deferred summary: `GET /api/v1/budgets/summary?month=YYYY-MM` | None on list screen; setup link goes to `/budget/setup` | Month key is required; budget amounts are decimal strings; progress percentage may exceed 100 for over-budget display | Summary skeleton and category-card skeletons | No budget configured for selected month with setup action | Retry summary/list load |
| `/budget/setup` default allocation form | Monthly income, default tab, allocation summary, fixed category budget inputs, save budget | Implemented dependency: `GET /api/v1/categories?kind=expense`; deferred setup template: `GET /api/v1/budgets/setup?month=YYYY-MM` | Deferred setup mutation: `PUT /api/v1/budgets/setup`; individual budgets can be created with `POST /api/v1/budgets` | Monthly income required and decimal >= 0; category allocations decimal >= 0; total allocated cannot exceed income unless the product explicitly allows it; category ids must exist | Existing setup skeleton; category list skeleton | No categories available; show category setup guidance | Field validation and submit retry |
...
| Screen | Visible data | Required query | Required mutation | Validation | Loading state | Empty state | Error state |
|---|---|---|---|---|---|---|---|
| `/savings` | Total savings `$2,483.39`, active goal count, target total `$7,500.00`, overall progress, all/active/completed filter, repeated savings goal cards | Implemented: `GET /api/v1/savings-goals?status=all|active|completed&cursor=&limit=`; deferred summary: `GET /api/v1/savings-goals/summary` | Create link should route with real create mode; drawer delete is `DELETE /api/v1/savings-goals/{id}` | Status enum; progress percentage may exceed 100 for over-target display; amounts decimal strings | Summary skeleton and card-list skeleton | No savings goals with create action | Retry summary/list load; inline delete failure |
