...
| `/auth` email entry | Email field, continue button, Google and GitHub buttons | None; OAuth provider discovery starts through the mobile BFF | Route email users to login/register; start Google/GitHub OAuth through `/api/auth/oauth/[provider]/start` | Email format required; only supported OAuth providers accepted | Submit/redirect spinner | Not applicable | Email validation or OAuth-start failure |
...
| `/auth/oauth/callback` | OAuth completion/loading or provider error | None; consumes the provider result from the URL fragment | Existing account: `/api/auth/oauth/exchange`; new account: hold the opaque registration ticket in session storage before redirecting to `/auth/register?oauth=1` | Provider, one-time exchange code, and safe return path validated | Callback spinner | Not applicable | Provider denial, expired/invalid code or ticket, session failure |
