...
| `/settings/security` | Password status plus authoritative Google and GitHub connection states | `GET /api/v1/auth/methods` through the same-origin backend proxy | `POST /api/auth/oauth/{provider}/link` creates a user/provider-bound intent; `PUT /api/auth/password` sets or changes the password and clears local session cookies | Supported provider only; connected identities cannot be replaced; current password required when one exists; 12–128 characters with uppercase, lowercase, and a number | Sign-in-method skeleton and pending connect/password actions | Disconnected provider and password-not-set states expose explicit actions | Provider collision/callback result, expired session redirect, password validation, and retryable load/mutation errors |
