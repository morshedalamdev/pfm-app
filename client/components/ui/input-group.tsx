...
const inputGroupButtonVariants = cva(
  "text-sm shadow-none flex gap-2 items-center",
  {
    variants: {
      size: {
        xs: "h-4 gap-1 px-2 rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-3.5 has-[>svg]:px-2",
        sm: "h-6 px-2.5 gap-1.5 rounded-md has-[>svg]:px-2.5",
        "icon-xs":
...
function InputGroupTextarea({
  ...
  return (
    <Textarea
      data-slot="input-group-control"
      className={cn(
        "flex-1 resize-none rounded-none border-0 bg-transparent py-2 px-2 shadow-none focus-visible:ring-0 dark:bg-transparent text-sm",
        className
      )}
      {...props}
    />
